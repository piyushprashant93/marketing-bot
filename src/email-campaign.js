/**
 * SutraWeave - Email Marketing Campaign
 *
 * Sends customized HTML marketing emails to Patna businesses
 * using Mailtrap SMTP. Each email is tailored to the business category.
 *
 * Usage:
 *   node src/email-campaign.js            → Send emails
 *   node src/email-campaign.js --dry-run  → Preview without sending
 *   node src/email-campaign.js --test     → Send test email to yourself
 *
 * Prerequisites:
 *   - Set MAIL_PASSWORD in .env with your Mailtrap API token
 *   - Verify your sending domain in Mailtrap dashboard
 */
...
const CONTACTS_FILE = path.join(__dirname, "..", "data", "contacts_enriched.csv");
...
function loadSentEmails() {
  try {
    if (fs.existsSync(EMAIL_SENT_FILE)) {
      ...
      return new Set((data.sent || []).map((e) => e.email || e));
    }
  } catch (err) {
    // Start fresh
  }
...
function markEmailSent(email, details = {}) {
  ...
  fs.writeFileSync(
    EMAIL_SENT_FILE,
    JSON.stringify(
      { sent: records, totalSent: records.length, lastUpdated: new Date().toISOString() },
      null,
      2
    )
  );
...
const categoryContent = {
  restaurant: {
    ...
    bullets: [
      "Auto-share your menu, specials & timings",
      "Take table reservations 24/7 via AI bot",
      "Handle delivery queries automatically",
    ],
  ...
  clinic: {
    ...
    bullets: [
      "Book patient appointments automatically",
      "Share doctor schedules & specializations",
      "Answer treatment & fee FAQs 24/7",
    ],
  ...
  hospital: {
    ...
    bullets: [
      "Share department info & doctor schedules instantly",
      "Handle appointment queries automatically",
      "Provide emergency contact info 24/7",
    ],
  ...
  salon: {
    ...
    bullets: [
      "Show services & pricing instantly",
      "Book appointments without back-and-forth",
      "Share your latest portfolio automatically",
    ],
  ...
  gym: {
    ...
    bullets: [
      "Share membership plans & pricing automatically",
      "Answer timing & batch schedule queries",
      "Capture new member leads 24/7",
    ],
  ...
  hotel: {
    ...
    bullets: [
      "Share room types & availability instantly",
      "Provide tariff details automatically",
      "Answer amenity & check-in queries 24/7",
    ],
  ...
  real_estate: {
    ...
    bullets: [
      "Share property listings with photos & pricing",
      "Schedule site visits automatically",
      "Qualify leads before you pick up the phone",
    ],
  ...
  retail: {
    ...
    bullets: [
      "Share product catalog & pricing instantly",
      "Answer store timing & location queries",
      "Handle order status automatically",
    ],
  ...
  pharmacy: {
    ...
    bullets: [
      "Confirm medicine availability instantly",
      "Share store timings & delivery options",
      "Handle prescription queries automatically",
    ],
  ...
  automobile: {
    ...
    bullets: [
      "Share vehicle models, features & pricing",
      "Book test drives & service appointments",
      "Handle loan/EMI queries automatically",
    ],
  ...
  events: {
    ...
    bullets: [
      "Share portfolio & packages instantly",
      "Check date availability automatically",
      "Provide customized quotations 24/7",
    ],
  ...
  services: {
    ...
    bullets: [
      "Share service packages & pricing instantly",
  ...
  travel: {
    ...
    bullets: [
      "Share tour packages & itineraries instantly",
      "Provide pricing & customization options",
      "Handle booking queries 24/7",
    ],
...
const defaultContent = {
  ...
  bullets: [
    ...
    "Share your service details instantly",
...
function buildEmailHTML(businessName, category) {
  ...
  const bulletsHTML = content.bullets
    .map(
      (b) =>
        `<tr><td style="padding:4px 0;color:#334155;font-size:15px;">✅ ${b}</td></tr>`
    )
    .join("\n");
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <h2 style="margin:16px 0;color:#0f172a;font-size:22px;font-weight:700;line-height:1.3;">
                ${content.headline}
              </h2>
              ...
              <div style="background:#f0f9ff;border-left:4px solid #3b82f6;padding:16px 20px;border-radius:0 8px 8px 0;margin:0 0 32px;">
                <p style="margin:0;color:#1e40af;font-size:14px;line-height:1.5;font-style:italic;">
                  💡 ${content.cta}
                </p>
              </div>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="https://sutraweave.sutracode.in/?ref=email&biz=${encodeURIComponent(businessName)}" 
                       style="display:inline-block;background:linear-gradient(135deg,#3b82f6,#1d4ed8);color:#ffffff;text-decoration:none;padding:14px 40px;border-radius:50px;font-size:16px;font-weight:600;letter-spacing:0.3px;">
          ...
          <tr>
            <td style="background:#f8fafc;padding:24px 40px;border-top:1px solid #e2e8f0;">
              <p style="margin:0 0 8px;color:#64748b;font-size:13px;">
                Best regards,<br>
                <strong>Piyush Prashant</strong><br>
                Founder, SutraCode
              </p>
              ...
              <p style="margin:12px 0 0;color:#cbd5e1;font-size:10px;">
                You received this email because your business is listed on Google Maps in Patna.
                If you don't want further emails, simply reply with "unsubscribe".
              </p>
            </td>
          </tr>
...
async function main() {
  ...
  // Validate config
  if (!MAIL_PASS || MAIL_PASS.includes("YOUR_")) {
  ...
  if (DRY_RUN) console.log(chalk.yellow.bold("  ⚠️  DRY RUN — No emails sent\n"));
  if (TEST_MODE) console.log(chalk.magenta.bold("  🧪 TEST MODE\n"));
  ...
  if (!DRY_RUN) {
    try {
      ...
      console.log(chalk.yellow("  Check your MAIL_PASSWORD in .env\n"));
      process.exit(1);
    }
  }
  ...
  if (TEST_MODE) {
    ...
    if (!DRY_RUN) {
      ...
      console.log(chalk.yellow("  DRY-RUN: Would send test email."));
      console.log(chalk.gray(`  Subject: ${content.subject}`));
    }
    return;
  }
  ...
  const allContacts = await loadCSV(CONTACTS_FILE);
  const withEmail = allContacts.filter(
    (c) => c.email && c.email.trim() && !c.email.includes("example.com")
  );

  const sentEmails = loadSentEmails();
  const unsent = withEmail.filter((c) => {
    // Check all emails for this contact
    const emails = c.email.split(",").map((e) => e.trim());
    return emails.some((e) => !sentEmails.has(e));
  });

  console.log(chalk.white(`  📋 Total contacts:     ${allContacts.length}`));
  console.log(chalk.white(`  📧 With email:          ${withEmail.length}`));
  console.log(chalk.white(`  ✉️  Unsent:             ${unsent.length}`));
  console.log(chalk.gray(`  🚫 Already sent:        ${withEmail.length - unsent.length}\n`));

  if (unsent.length === 0) {
    console.log(chalk.yellow("⚠️  No unsent emails! Delete data/emails_sent.json to resend.\n"));
    return;
  }

  // Send campaign
  const toSend = unsent.slice(0, MAX_EMAILS);
  console.log(
    chalk.cyan.bold(`━━━ Sending ${toSend.length} emails ━━━\n`)
  );
  ...
  for (let i = 0; i < toSend.length; i++) {
    ...
    const email = contact.email.split(",")[0].trim(); // Use first email
    ...
    if (DRY_RUN) {
      console.log(
        `${progress} ${chalk.yellow("DRY-RUN")} → ${chalk.white(businessName)} ${chalk.gray(`[${category}]`)} <${email}>`
      );
      console.log(chalk.gray(`   Subject: ${content.subject}`));
      continue;
    }
    ...
    try {
      process.stdout.write(`${progress} 📧 ${businessName.substring(0, 35).padEnd(35)} → ${email} ...`);
  ...
  console.log(chalk.blue.bold("  📊 Email Campaign Summary"));
  console.log(chalk.blue("═══════════════════════════════════════"));
  if (DRY_RUN) {
    console.log(chalk.yellow(`  Mode:       DRY RUN`));
    console.log(chalk.white(`  Previewed:  ${toSend.length} emails`));
  } else {
    console.log(chalk.green(`  ✅ Sent:     ${sent}`));
    console.log(chalk.red(`  ❌ Failed:   ${failed}`));
  }
  console.log(chalk.gray(`  📋 Check logs: https://mailtrap.io/sending/email_logs`));
  console.log(chalk.blue("═══════════════════════════════════════\n"));
}
